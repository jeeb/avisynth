...
//
//  this is a modifed version of the original source-code.
//  this source-code is a win32 dll implementation of SSRC, made by DSPguru.
//  code last modified : 25/02/2002
//
//  released under LPGL.
//
// contact information :
// Naoki Shibata - shibatch@geocities.co.jp
...
SSRC_API int init_shaper(int freq,int nch,int min,int max,int dtype)
{
  ...
  randbuf = (int *)malloc(sizeof(int) * RANDBUFLEN);
...
SSRC_API int do_shaping(double s,double *peak,int dtype,int ch)
{
  ...
  if (dtype == 1) {
    s += 1.0*(((double)randbuf[randptr++ & (RANDBUFLEN-1)]/RAND_MAX)-0.5);
  ...
  s += h;
  u = s;
  s += 0.7*(((double)randbuf[randptr++ & (RANDBUFLEN-1)]/RAND_MAX)-0.5);
  if (s < shaper_clipmin) {
    double d = (double)s/shaper_clipmin;
    *peak = *peak < d ? d : *peak;
    s = shaper_clipmin;
...
{
  return izero(alp*sqrt(1-4*n*n/((len-1)*(len-1))))/iza;
}
...
REAL *op;
int n1b2,n1b;
int n2x,n2y;   // n2y = fs2/fs1;
int dfrq,sfrq;
int frqgcd,osf,fs1,fs2; //  fs1 = sfrq*osf ; fs2 = sfrq / frqgcd * dfrq ;
...
//  int n2,n2x,n2y,n1,n1b;
  int n2,n1;
  int filter1len = 16384; /* stage 1 filter length */
  int i,j;
  ...
  *ENDING=&ending;
...

  {
    ...
    stage1 = (float *)malloc(sizeof(REAL)*n1b);
    ...
    fft_ip[0] = 0;
    wsize     = n1b/2;
    fft_w     = (float *)malloc(sizeof(REAL)*wsize);
  ...
  if (osf == 1) {
    ...
    f2inc[0] = sfrq/dfrq;
    stage2 = (float **)malloc(sizeof(REAL *)*n2y);
    stage2[0] = (float *)malloc(sizeof(REAL)*n2x*n2y);
    stage2[0][0] = 1;
  } else {
    double aa = AA; /* stop band attenuation(dB) */
    double lpf,delta,d,df,alp,iza;
    ...
    buf1 = (float **)malloc(sizeof(REAL *)*nch);
    for(i=0;i<nch;i++)
      buf1[i] = (float *)malloc(n1b*sizeof(REAL));

    buf2 = (float **)malloc(sizeof(REAL *)*nch);
    for(i=0;i<nch;i++) {
      buf2[i] = (float *)malloc(sizeof(REAL)*(n2x+1+n1b2));
    ...
    outbuf = (float *)malloc(sizeof(REAL)*nch*((double)n1b2*sfrq/dfrq+1));
//	toberead=(n1b2-rps-1)/osf+1;
	toberead=nch*((n1b2-rps-1)/osf+1);
    op = outbuf;
	return ( &toberead) ;
}
...
#else
SSRC_API void downsample(REAL *inbuf,/*REAL* outbuf,*/int nsmplread,int init,int ending)
#endif
{
  static int i,j;
  static int _;
...
//		int s;
	...
	{	
	  if (ending) 
	  {
		...
		{
	      deliver(outbuf,nch*nsmplwrt2);
	      sumwrite += nsmplwrt2;
	    } 
		else 
		{
	      deliver(outbuf,nch*(floor((double)sumread*dfrq/sfrq)+2-sumwrite));
	      break;
	    }
	  } 
	  else 
	...
	else
	{
	  ...
	  else
	  {
	    if (ending) 
		{
		  ...
		  {
			deliver(outbuf+nch*delay,nch*(nsmplwrt2-delay));
			sumwrite += nsmplwrt2-delay;
	      } 
		  else  
		  {
			deliver(outbuf+nch*delay,nch*(floor((double)sumread*dfrq/sfrq)+2-sumwrite-delay));
			break;
	      }
	    } 
		else 
...
int SSRC_API deinit_ssrc()
{
	int i;
  if(stage1) free(stage1);
  if(fft_ip) free(fft_ip);
  if(fft_w) free(fft_w);
  if(f2order) free(f2order);
  if(f2inc) free(f2inc);
  if(stage2) 
  {
	  free(stage2[0]);
...
double downsample(FILE *fpi,FILE *fpo,int nch,int bps,int dbps,int sfrq,int dfrq,double gain,int chanklen,int twopass,int dither)
{
  int frqgcd,osf,fs1,fs2;
  REAL *stage1,**stage2;
  int n2,n2x,n2y,n1,n1b;
...
void usage(void)
{
  printf("http://www.geocities.co.jp/Technopolis/9674/ssrc/\n\n");
  ...
  printf("                                       0 : no dither\n");
  printf("                                       1 : add white noise\n");
  printf("                                       2 : triangular dither\n");
  printf("                                       3 : ATH based noise shaping\n");
  printf("                                       4 : less dither amplitude than type 3\n");
}

double upsample(FILE *fpi,FILE *fpo,int nch,int bps,int dbps,int sfrq,int dfrq,double gain,int chanklen,int twopass,int dither)
{
  int frqgcd,osf,fs1,fs2;
  REAL **stage1,*stage2;
  int n1,n1x,n1y,n2,n2b;
  int filter2len = 16384; // stage 2 filter length 
  int *f1order,*f1inc;
  int *fft_ip = NULL;
  REAL *fft_w = NULL;
  unsigned char *rawinbuf,*rawoutbuf;
...
int main(int argc,char **argv)
{
  char *sfn,*dfn;
  FILE *fpi,*fpo,*fpt;
  int twopass,normalize,dither,samp;
  int nch,bps,length;
  int sfrq,dfrq,dbps;
  double att,peak;
  int i,j;
  ...
  if (dither == -1) {
    if (dbps < bps) dither = 3; else dither = 1;
  }

  if (!quiet) {
    const char *dtype[] = {
      "none","white noise","triangular","ATH based noise shaping","ATH based noise shaping(less amplitude)"
    };
  ...
  if (!fpo) {fprintf(stderr,"cannot open input file.\n"); exit(-1);}
  ...
  if (dither) {
    ...
    samp = init_shaper(dfrq,nch,min,max,dither);
  }

  if (twopass) {
    REAL gain;
    int ch=0,fptlen,sumread;
