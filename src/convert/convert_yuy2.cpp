...
/* Original code for 0-1-1 & 0-1-0 kernels */
...
#if 0
/* Unwrapped code for analysis, 1-2-1 kernel */
...
// mm0 mm1 mm2 mm3 mm4 mm5 mm6 mm7
  ...
  x86.movd(      mm3,dword_ptr[SRC+RGBOFFSET]); //mm3= 0000 0000 XXR1 G1B1
  x86.punpcklbw( mm3,mm3);            // mm1= XXXX R1R1 G1G1 B1B1
  x86.psrlw(     mm3,8);               // mm1= 00XX 00R1 00G1 00B1
  x86.movq(      mm5,mm3);             // mm5= 00XX 00R0 00G0 00B0
  ...
  x86.psrlq(     mm4,32);
  x86.paddd(     mm3,mm4);
  x86.psrld(     mm3,15);              // mm1= xxxx xxxx 0000 00y0 final value
  ...
  x86.paddw(     mm5,mm1);             // mm5 += 00XX 00R1 00G1 00B1
  x86.paddw(     mm5,mm1);             // mm5 += 00XX 00R1 00G1 00B1
  x86.paddw(     mm5,mm2);             // mm5  += 00XX 00R2 00G2 00B2 accumulated RGB values (for b_y and r_y)

  x86.movq(      mm6,qword_ptr[&rb_mask]); // Clear out accumulated G-value mm6= 0000 RRRR 0000 BBBB
  x86.pand(      mm6,mm5);             // mm5= 00XX 00R0 00G0 00B0
  ...
  if (sub) {
    x86.paddw(   mm3,qword_ptr[&sub_64]);// mm3 = y0 - 64
  }
  ...
  if (sub) {
    x86.movd(    mm4,dword_ptr[dyn_y1y2_mult]);//0x00004A85,    //=(255./219.) << 14
    x86.pmaddwd( mm3,mm4);             // mm3=scaled_y(latency 2 cycles)
    x86.punpckldq(mm3,mm3);            // Move scaled_y to upper dword mm3=SCALED_Y SCALED_Y
    x86.pslld(   mm6,14);              // Shift up accumulated R and B values (<<15 in C)
    x86.psubd(   mm6,mm3);             // mm6 = b_y and r_y
  ...
  x86.pmaddwd(   mm6,qword_ptr[dyn_fpix_mul]); // Mult b_y and r_y 0x0000503300003F74, =(1/((1-0.299)*255/112)<<15+0.5), (1/((1-0.114)*255/112)<<15+0.5)
  x86.paddd(     mm6,qword_ptr[&fpix_add]); // Add 0x808000 to r_y and b_y
  x86.pand(      mm6,qword_ptr[&chroma_mask2]); // Clear out fractions
  x86.packuswb(  mm6,mm6);             // mm6 = VV00 UU00 VV00 UU00
