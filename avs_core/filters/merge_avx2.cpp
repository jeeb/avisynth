...
void average_plane_avx2(BYTE *p1, const BYTE *p2, int p1_pitch, int p2_pitch, int rowsize, int height) {
  ...
  for(int y = 0; y < height; y++) {
    for(int x = 0; x < mod32_width; x+=32) {
      __m256i src1  = _mm256_load_si256(reinterpret_cast<const __m256i*>(p1+x));
      __m256i src2  = _mm256_load_si256(reinterpret_cast<const __m256i*>(p2+x));
      __m256i dst;
      if(sizeof(pixel_t)==1)
...
void weighted_merge_planar_uint16_avx2(BYTE *p1, const BYTE *p2, int p1_pitch, int p2_pitch, int rowsize, int height, int weight, int invweight) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < wMod32; x += 32) {
      __m256i px1, px2;
      px1 = _mm256_load_si256(reinterpret_cast<__m256i*>(p1 + x)); // y7 y6 y5 y4 y3 y2 y1 y0
      px2 = _mm256_load_si256(const_cast<__m256i*>(reinterpret_cast<const __m256i*>(p2 + x))); // Y7 Y6 Y5 Y4 Y3 Y2 Y1 Y0
